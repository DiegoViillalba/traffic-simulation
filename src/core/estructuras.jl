...
mutable struct Auto
    ...
    esquinas ::Vector{Vector{Float64}}
...
function Auto(ancho::Union{Float64,Int64},largo::Union{Float64,Int64},posicion::Vector{Float64},indice::Int64,)
    esquinas = Vector{Vector{Float64}}([
        [posicion[1]-ancho/2, posicion[2]+largo/2],
        [posicion[1]+ancho/2, posicion[2]+largo/2],
        [posicion[1]+ancho/2, posicion[2]-largo/2],
        [posicion[1]-ancho/2, posicion[2]-largo/2]
    ])
...
function +(a::Auto, x::Array)
    a.posicion .+= x
    a.esquinas = Vector{Vector{Float64}}([a.esquinas[i] .+ x for i in 1:4])
    return a
end
...
function mod(a::Auto, x)
    ...
    bp = a.posicion .- [0.0, x*factor]
    bes = Vector{Vector{Float64}}([es .- [0.0, x*factor] for es in a.esquinas])
