...
Ejemplos:
    ...
    julia --threads=auto launch.jl --config configs/alta_densidad.yaml --pasos 1000
...
function parsear_args(args)
    params = Dict{String,Any}(
        ...
        "fps"            => 12,
        "calentamiento"  => 0,
        "pasos"          => nothing,   # nothing = usar el valor del YAML
        "benchmark"      => false,
        "grilla"         => true,
        "output"         => "results",
        "lista"          => false,
    ...
    while i <= length(args)
        ...
        elseif arg == "--sin-grilla"
            params["grilla"] = false
        elseif arg in ("--config", "--fps", "--calentamiento", "--pasos", "--output")
            if i >= length(args)
...
function mostrar_ayuda()
    ...
    Ejemplos:
        ...
        julia launch.jl --config configs/alta_densidad.yaml
        julia launch.jl --config configs/baja_densidad.yaml --animacion --fps 15
        julia launch.jl --config configs/baja_densidad.yaml --calentamiento 500 --animacion
...
function listar_configs()
    ...
    for f in sort(archivos)
        ...
        try
            ...
            println("  $(rpad(f, 28))  [$tipo]  $(n)+$(m) autos  $(pasos) pasos")
            println("  $(repeat(' ', 28))  $desc")
            println()
        catch
            println("  $f")
        end
...
if hacer_benchmark
    ...
    println("  carros_i_carriles (1 llamada)   : $(round(t_ci_ms, digits=3)) ms  -> x$(n+m) = $(round(t_ci_ms*(n+m),digits=2)) ms/paso")
    println("  listas_carros_fantasmas (1 llam): $(round(t_fan_ms, digits=3)) ms  -> x$(n+m) = $(round(t_fan_ms*(n+m),digits=2)) ms/paso")
    ...
    if get(_res, "guardar_benchmarks", true)
        ...
        println()
        println("  benchmarks.csv guardado.")
    end

    # Reinicializar para la simulacion
...
if hacer_animacion
    ...
    println("  Simulacion completada en $(round(t_total, digits=2)) s")
    println("  GIF guardado en: $gif_path")
else
    println("Ejecutando simulacion ($pasos pasos)...")
    t_total = @elapsed begin
    ...
    rho1 = round(n / (3.5 * L), digits=4)
    rho2 = round(m / (3.5 * L), digits=4)
    ...
    println("  Velocidad global  : $(round(mean(V),digits=4)) u/paso")
    println("  Densidad carril 1 : rho1 = $rho1 veh/m")
    println("  Densidad carril 2 : rho2 = $rho2 veh/m")

    if get(_res, "guardar_velocidades", true)
        df_vel = DataFrame(tiempo=T, velocidad_promedio=V)
        CSV.write(joinpath(results_dir, "velocidades.csv"), df_vel)
        println("  velocidades.csv guardado.")
    end
end
...
println()
println("Resumen guardado en: $(joinpath(results_dir, "resumen.yaml"))")
println()
